...
Original file is located at
    https://colab.research.google.com/drive/1lLq3AjzBaa_lgJ9DjuHDpeechhDW1vw2
"""
...
def train(dataloader, model, loss_fn, optimizer, epoch, writer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        if batch % 10 == 0 and torch.cuda.is_available():
        ...
        optimizer.step()
        optimizer.zero_grad()  # Move this before step()
        
        # Free up memory
        del pred
        if batch % 100 == 0:
            loss = loss.item()  # Get loss value before deleting
            del loss
...
warmup_scheduler = LinearLR(optimizer, 
                          start_factor=0.1,
                          total_iters=5 * len(train_loader))  # 5 epochs warmup

main_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 
...
try:
    for epoch in range(start_epoch, 100):
        ...
        # Save epoch checkpoint only every 5 epochs to save disk space
        if epoch % 5 == 0:
    ...
    print(f"Training interrupted at epoch {epoch}: {str(e)}")
    ...
    torch.save(checkpoint, os.path.join("checkpoints", params.name, f"emergency_checkpoint.pth"))
finally:
    writer.close()